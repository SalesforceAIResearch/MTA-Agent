...
"""
Preprocess the Infoseek dataset to parquet format for model training (Qwen3VL version)
"""
import fire
import json
import datasets
...
def process_scenarios(
    ...
    for idx, scenario in enumerate(scenarios):
        try:
            ...
            data_id = scenario.get('id') or scenario.get('qid') or scenario.get('question_id') or f'infoseek_{start_idx + idx}'
...
# python -c 'import json; print(len(json.load(open("/export/share/beckypeng/mm_dr/data/Infoseek/qa_formatted_train.json"))["scenarios"]))'
def main(
    qa_json_path: str = '/export/share/beckypeng/mm_dr/data/Infoseek/qa_formatted_train.json',
    images_dir: str = '/export/share/beckypeng/mm_dr/data/Infoseek/images',
    split: str = None,  # 'train', 'val', 'test', or None/'all' to process all splits
    local_dir: str = '/export/home/becky/verl-tool-mm-deepsearch/data/mm_deepsearch/infoseek_qw3vl',
    image_sep: str = "<image>",
    filter_len: int = None,
    image_url_prefix: str = 'https://cqin-public-data.s3.us-west-1.amazonaws.com/mm-deepsearch-sfr-2025/Infoseek/images/',
    ...
    exclusion_file: str = "/export/home/becky/verl-tool-mm-deepsearch/verl_step_records/multimodal_deepsearch-fsdp-agent-_export_share_beckypeng_models_qwen3-vl-8b-instruct-grpo-n8-b16-t1.0-lr1e-6-mm_deepsearch_multi-mmresearch-8b-poc-20260106-202324/rollout_summary.json",  # Path to rollout_summary.json to filter out high-accuracy pairs
    max_output: int = None,  # Maximum number of examples to output per split (applied after all filtering)
    val_parquet: str = "/export/home/becky/verl-tool-mm-deepsearch/data/mm_deepsearch/infoseek_qw3vl/val.parquet",  # Path to existing val.parquet - questions will be excluded from train
    test_parquet: str = "/export/home/becky/verl-tool-mm-deepsearch/data/mm_deepsearch/infoseek_qw3vl/test.parquet",  # Path to existing test.parquet - questions will be excluded from train
):
    """
    Preprocess Infoseek dataset to parquet format compatible with Qwen3VL model training.
    ...
    exclusion_set = load_exclusion_list(exclusion_file, 'Infoseek') if exclusion_file else set()
    ...
    if isinstance(data, list):
        ...
        scenarios = data.get('scenarios', data.get('data', data.get('questions', [])))
    else:
...
python examples/data_preprocess/mm_search/infoseek_qw3vl.py --qa_json_path=data/mm_deepsearch/Infoseek/qa_formatted.json --images_dir=data/mm_deepsearch/Infoseek/images --local_dir=data/mm_deepsearch/infoseek_qw3vl

# Process only train split
python examples/data_preprocess/mm_search/infoseek_qw3vl.py --qa_json_path=data/mm_deepsearch/Infoseek/qa_formatted.json --images_dir=data/mm_deepsearch/Infoseek/images --split=train --local_dir=data/mm_deepsearch/infoseek_qw3vl

# Process with content length filter
python examples/data_preprocess/mm_search/infoseek_qw3vl.py --qa_json_path=data/mm_deepsearch/Infoseek/qa_formatted.json --images_dir=data/mm_deepsearch/Infoseek/images --local_dir=data/mm_deepsearch/infoseek_qw3vl --filter_len=8192

# Process with exclusion (filter out high-accuracy questions from rollout_summary.json)
# Output files will have '_filtered' suffix, e.g., train_filtered.parquet
python examples/data_preprocess/mm_search/infoseek_qw3vl.py --qa_json_path=data/mm_deepsearch/Infoseek/qa_formatted.json --images_dir=data/mm_deepsearch/Infoseek/images --local_dir=data/mm_deepsearch/infoseek_qw3vl --exclusion_file=verl_step_records/.../rollout_summary.json

# Process with exclusion and limit output to max 1000 examples per split
python examples/data_preprocess/mm_search/infoseek_qw3vl.py --qa_json_path=data/mm_deepsearch/Infoseek/qa_formatted.json --images_dir=data/mm_deepsearch/Infoseek/images --local_dir=data/mm_deepsearch/infoseek_qw3vl --exclusion_file=verl_step_records/.../rollout_summary.json --max_output=1000
"""
