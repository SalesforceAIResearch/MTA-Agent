...
class DataProto:
    ...
    def concat(data: list["DataProto"]) -> "DataProto":
        ...
        # Union of all keys so workers with different reward_extra_keys still concat to same batch size
        all_nt_keys = set()
        for d in data:
            if d.non_tensor_batch:
                all_nt_keys.update(d.non_tensor_batch.keys())
        non_tensor_batch = {}
        for key in all_nt_keys:
            to_concat = []
            for d in data:
                if d.non_tensor_batch is not None and key in d.non_tensor_batch:
                    to_concat.append(d.non_tensor_batch[key])
                else:
                    # Pad missing key with None so concat length matches batch size
                    if d.batch is not None:
                        n = d.batch.batch_size[0]
                    elif d.non_tensor_batch:
                        n = len(next(iter(d.non_tensor_batch.values())))
                    else:
                        n = 0
                    to_concat.append(np.full((n,), None, dtype=object))
            non_tensor_batch[key] = np.concatenate(to_concat, axis=0)
        ...
        if data:
            ...
            for d in data:
                for k, v in d.meta_info.items():
                    if k == "metrics":
                        ...
                        if v is not None:
                            if isinstance(v, list):
                                all_reward_extra_keys.extend(v)
                            else:
                                all_reward_extra_keys.append(v)
                    else:
            ...
            # Union of reward_extra_keys from all workers, sorted for canonical order
            if all_reward_extra_keys:
                merged_meta_info["reward_extra_keys"] = sorted(set(all_reward_extra_keys))
