...
def compute_reward_extra_info_metrics(batch: DataProto) -> dict[str, Any]:
    """
    Computes metrics from reward_extra_info stored in batch.non_tensor_batch.
    
    This function extracts reward-related information (accuracy, score, format_penalty, 
    tool_call_reward, etc.) from reward_extra_info and computes mean statistics
    ...
    Returns:
        A dictionary of metrics with keys like:
            - train-reward/accuracy
            - train-reward/score
            - train-reward/format_penalty
            - train-reward/tool_call_reward
            - etc. for all keys in reward_extra_info
    """
    metrics = {}
    
    # Common reward extra info keys to look for
    reward_extra_info_keys = [
        "accuracy", "score", "format_penalty", "tool_call_reward",
        "think_format_penalty", "answer_format_penalty", "valid_action_penalty",
        "unfinished_traj_penalty", "no_tool_interact_penalty", "exec_error",
        "curiousity_penalty", "action_redundancy_penalty"
    ]
    
    # Check for any reward_extra_info keys in non_tensor_batch
    for key in reward_extra_info_keys:
        if key in batch.non_tensor_batch:
            ...
            if isinstance(values, np.ndarray) and values.size > 0:
                ...
                # Compute mean only
                metrics[f"train-reward/{key}"] = float(np.mean(values))
    
    # Also check for any other keys that might be in reward_extra_info
    # Look for keys that are numeric arrays
    for key, value in batch.non_tensor_batch.items():
        if key not in reward_extra_info_keys and key not in ["__num_turns__", "tool_call_counts", "request_id", "data_source", "reward_model", "extra_info", "uid"]:
            if isinstance(value, np.ndarray) and value.size > 0:
                ...
                try:
                    if np.issubdtype(value.dtype, np.number):
                        ...
                        # Only add if it looks like reward info (not too large arrays)
                        if value.ndim == 1 and len(value) < 10000:  # Reasonable size for reward info
                            metrics[f"train-reward/{key}"] = float(np.mean(value))
