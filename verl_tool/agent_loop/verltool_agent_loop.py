    ...
    async def run(self, sampling_params: dict[str, Any], **kwargs) -> AgentLoopOutput:
        ...
        for step in range(max_turns + 1):
            ...
            if is_last_step and self.action_stop_tokens:
                # Remove action stop tokens on last step to allow model to generate final answer
                # Keep other stop tokens (like EOS tokens)
            ...
            # On the last step, if model generated a tool call, ignore it and treat as final answer
            # The model should have been instructed to provide final answer, but if it didn't,
            # we accept whatever it generated as the final response
            if is_last_step and do_action:
                logger.warning(f"Turn {step} (LAST): Model generated tool call on last step. Treating generated text as final answer instead of executing tool.")
                do_action = False  # Don't execute the tool, treat the generation as final answer
                # The gen_text will be included in the response, which is what we want
