...
def strip_result_tags(s):
    """Remove <result> and </result> tags from a string (e.g. cached content)."""
    if s is None:
        return s
    return (s if isinstance(s, str) else str(s)).replace("</result>", "").replace("<result>", "")
