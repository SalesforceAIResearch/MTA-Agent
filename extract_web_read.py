#!/usr/bin/env python3
"""
Extract web_read entries from rollout JSONL files.
Extracts URL and response text for each web_read call.
Output format: {url: response}
"""
...
def is_valid_response(response: str) -> bool:
    ...
    error_patterns = [
        ...
        "not found",
        "web read timed out",
        "invalid web_read format",
        "unable to extract",
        "extraction failed",
        "no content extracted from urls",
    ...
    for pattern in error_patterns:
        if pattern in response_lower:
            # Always ignore "No content extracted from URLs" regardless of length
            if pattern == "no content extracted from urls":
                return False
            if pattern == "no content extracted":
                return False
            return False
...
def extract_web_read_from_line(line: str, data: Dict[str, Any] = None) -> List[Dict[str, Any]]:
    """
    Extract web_read entries from a single JSONL line.
    Returns a list of dictionaries with url, question, and response text.
    """
    results = []
    ...
    try:
        ...
        print(f"Error: Failed to parse JSON in extract_web_read_from_line: {e}")
        return results
    ...
    if "tool_interact_info" in data and isinstance(data["tool_interact_info"], list):
        for tool_info in data["tool_interact_info"]:
            if isinstance(tool_info, dict):
                ...
                # Check if action contains <web_read> tag
                has_web_read = False
                if isinstance(action_str, str) and "<web_read>" in action_str:
                    has_web_read = True
                
                if has_web_read:
                    try:
                        # Extract URL from action field
                        # Action might be in format: <web_read>url</web_read>
                        url = None
                        response_text = None
                        
                        if isinstance(action_str, str):
                            # Extract content between tags
                            content_match = re.search(r'<web_read>(.*?)</web_read>', action_str, re.DOTALL)
                            if content_match:
                                url = content_match.group(1).strip()
                            else:
                                print(f"Error: Could not extract URL from action: {action_str[:100]}...")
                        ...
                        if "Response:" in full_obs_text:
                            ...
                            if len(parts) > 1:
                                ...
                                # If split didn't work as expected, use full text
                        ...
                        if url:
                            # Build cache_key with question: url||question or just url
                            if question:
                                cache_key = f"{url}||{question.lower()}"
                            else:
                                cache_key = url
                            ...
                            print(f"Error: Missing URL for web_read entry (input_id: {data.get('input_id')}, step: {data.get('step')})")
                    except Exception as e:
                        print(f"Error: Exception while processing web_read entry: {e}")
...
def process_rollout_file(file_path: str) -> List[Dict[str, Any]]:
    """
    Process a single rollout JSONL file and extract all web_read entries.
    ...
    try:
        ...
        while idx < len(content):
            ...
            try:
                ...
                results = extract_web_read_from_line(json_str, obj)
                if results:
                    all_results.extend(results)
                    entry_count += len(results)
                    print(f"  Found {len(results)} web_read entry/entries (entry #{entry_count})")
                idx = end_idx
...
def main():
    """
    Main function to extract web_read entries from rollout files.
    """
    parser = argparse.ArgumentParser(
        description="Extract web_read entries from rollout JSONL files",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Process default verl_step_records directory
  python extract_web_read.py
  
  # Process a single folder
  python extract_web_read.py -d verl_step_records
  
  # Process multiple folders
  python extract_web_read.py -d folder1 folder2 folder3
  
  # Specify output file
  python extract_web_read.py -d verl_step_records -o output.json
  
  # Verbose mode
  python extract_web_read.py -d verl_step_records -v
        """
    )
    ...
    parser.add_argument(
        '-o', '--output',
        default='web_read_extracted.json',
        help='Output JSON file path (default: web_read_extracted.json)'
    )
    ...
    print()
    print(f"Total web_read entries found: {len(all_results)}")
    
    # Convert to simple dictionary format: cache_key -> response
    # cache_key format: "url||question" or "url"
