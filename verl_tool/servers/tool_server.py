...
class AsyncToolManager:
    ...
    def _initialize_tools(self, tool_types: Tuple[str]) -> None:
        ...
        finish_failed = any(tool_type == "finish" for tool_type, _ in failed_tools)
    ...
    def _identify_tool_for_action(self, action: str, extra_field: Dict[str, Any]) -> Optional[str]:
        ...
        # Get non-special tools (exclude finish, mcp_interface, base)
        non_special_tools = {
            k: v for k, v in self.tools.items() 
            if k not in ["finish", "mcp_interface", "base"]
        }
        
        # Single tool case - return immediately
        if len(non_special_tools) == 1:
            return list(non_special_tools.keys())[0]
        
        # Multiple tools - try to parse with each
        parse_errors = []
        for tool_type, tool in non_special_tools.items():
            try:
        ...
        if not any(tag in action for tag in tool_tags):
