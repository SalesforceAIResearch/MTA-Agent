...
from .google_search_tavily import AsyncLRUCache
...
@register_tool
class GoogleSearchTavilyImageDescriptionTool(BaseTool):
    """
    Async Google image search tool using Tavily API with image descriptions.
    """
    
    tool_type = "google_search_tavily_image_description"
    ...
    def get_usage_inst(self):
        ...
        return "Search for images using Tavily API and get their descriptions. Use <search_image>your query</search_image> format."
    ...
    async def _conduct_action_async(self, trajectory_id: str, action: str, extra_field: Dict[str, Any]) -> Tuple[str, bool, bool]:
        ...
        if not is_valid:
            observation = "Invalid search query. Use <search_image>your query</search_image> format."
            done, valid = False, False
        else:
...
if __name__ == "__main__":
    ...
    # Test basic search
    tool = GoogleSearchTavilyImageDescriptionTool(api_key=api_key, max_results=2)
    
    # Test query parsing
    query, valid = tool.parse_action("<search_image>Python programming</search_image>")
    print(f"Parsed query: '{query}', Valid: {valid}")
    
    # Test search execution
    observation, done, valid = tool.conduct_action("test", "<search_image>Shanghai EXPO</search_image>", {})
