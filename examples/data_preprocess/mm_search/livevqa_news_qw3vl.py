...
"""
Preprocess the LiveVQA News dataset to parquet format for model training (Qwen3VL version)
"""
import fire
import json
import datasets
...
def extract_ground_truth(ground_truth_str, question_text=None):
    """
    Extract ground truth answers from the ground_truth string.
    
    Format: "Correct Answer: X\n\nAcceptable Answers: answer1, answer2, ...\n\nQuestion Type: ...\n\nImage URL: ..."
    
    Args:
        ground_truth_str: The ground truth string
    ...
    # Extract correct answer (the letter option)
    correct_answer_match = re.search(r'Correct Answer:\s*([A-E])', ground_truth_str)
    correct_letter = None
    if correct_answer_match:
        correct_letter = correct_answer_match.group(1)
        answers.append(correct_letter)
        
        # If we have question text, extract the full option text
        if question_text:
            # Match pattern like "A. Option text" or "A. Option text\n"
            option_pattern = rf'{re.escape(correct_letter)}\.\s*([^\n]+)'
            option_match = re.search(option_pattern, question_text)
            if option_match:
    ...
    acceptable_match = re.search(r'Acceptable Answers:\s*([^\n]+)', ground_truth_str)
    if acceptable_match:
        acceptable_text = acceptable_match.group(1).strip()
        # Split by comma and clean up
        acceptable_list = [ans.strip() for ans in acceptable_text.split(',') if ans.strip()]
        answers.extend(acceptable_list)
...
def extract_question_type(ground_truth_str):
    """Extract question type from ground_truth string."""
    if not ground_truth_str:
        return ''
    match = re.search(r'Question Type:\s*(\w+)', ground_truth_str)
    return match.group(1) if match else ''

def extract_image_url(ground_truth_str):
    """Extract image URL from ground_truth string."""
    if not ground_truth_str:
        return ''
    match = re.search(r'Image URL:\s*([^\n]+)', ground_truth_str)
...
def process_scenarios(
    ...
    for idx, scenario in enumerate(scenarios):
        try:
            # Extract fields
            data_id = scenario.get('id', f'livevqa_news_{start_idx + idx}')
            
            question_obj = scenario.get('question', {})
            if "level2" not in scenario["id"] and "4hop" not in scenario["id"] and "3hop" not in scenario["id"]:
                continue
            ...
            # Extract image path (relative path like "./cnn_88cb912daeafafdcc90ecfc624a6a2a9.jpg")
            image_path_rel = question_obj.get('image', '')
            if not image_path_rel:
                print(f"Warning: Skipping scenario {start_idx + idx} - no image path found")
                continue
            ...
            image_filename = image_path_rel.lstrip('./')
            ...
            # Save image to output directory with original filename
            img_path_output = output_image_dir / image_filename
            img.save(img_path_output, format='JPEG')
            ...
            ground_truth_str = scenario.get('ground_truth', '')
            if not ground_truth_str:
                print(f"Warning: Skipping scenario {start_idx + idx} - no ground truth found")
                continue
            
            ground_truth = extract_ground_truth(ground_truth_str, question_text)

            if not ground_truth and ground_truth_str:
            ...
            # Extract metadata
            question_type = extract_question_type(ground_truth_str)
            image_url_from_gt = extract_image_url(ground_truth_str)
            ...
            data_item = {
                ...
                "extra_info": {
                    ...
                    'num_images': 1,
                    'original_image_url': image_url_from_gt,
                }
            }
...
def main(
    qa_json_path: str = '/export/share/beckypeng/mm_dr/data/News/qa_formatted_train.json',
    images_dir: str = '/export/share/beckypeng/mm_dr/data/News/images',
    split: str = None,  # 'train', 'val', 'test', or None/'all' to process all splits
    local_dir: str = '/export/home/becky/verl-tool-mm-deepsearch/data/mm_deepsearch/livevqa_news_qw3vl',
    image_sep: str = "<image>",
    filter_len: int = None,
    image_url_prefix: str = 'https://cqin-public-data.s3.us-west-1.amazonaws.com/mm-deepsearch-sfr-2025/News/images/',
    ...
    max_output: int = 400,  # Maximum number of examples to output per split (applied after all filtering)
    val_parquet: str = "/export/home/becky/verl-tool-mm-deepsearch/data/mm_deepsearch/livevqa_news_qw3vl/val.parquet",  # Path to existing val.parquet - questions will be excluded from train
    test_parquet: str = "/export/home/becky/verl-tool-mm-deepsearch/data/mm_deepsearch/livevqa_news_qw3vl/test.parquet",  # Path to existing test.parquet - questions will be excluded from train
):
    """
    Preprocess LiveVQA News dataset to parquet format compatible with Qwen3VL model training.
    ...
    Args:
        ...
        image_url_prefix: URL prefix for image paths (default: S3 bucket URL)
        max_examples: Optional limit on number of examples to process (useful for testing)
        train_size: Number of examples for training split (default: 900)
        val_size: Number of examples for validation split (default: 20)
        test_size: Number of examples for test split (default: 80)
        seed: Random seed for splitting (default: 42)
        model_name: Qwen3VL model name (default: 'Qwen/Qwen3-VL-8B-Instruct')
        exclusion_file: Path to exclusion_list.json to filter out high-accuracy pairs (optional)
    """
    # Load exclusion list if provided
    exclusion_set = load_exclusion_list(exclusion_file, 'News') if exclusion_file else set()
    ...
    has_split_field = any('split' in scenario for scenario in scenarios)
    ...
    if not has_split_field:
        ...
        for scenario in scenarios:
            scenario_split = scenario.get('split', 'train')  # Default to 'train' if split field exists but is empty
...
"""
Example usage:
# Process all splits automatically (900 train, 20 val, 80 test)
python examples/data_preprocess/mm_search/livevqa_news_qw3vl.py --qa_json_path=data/mm_deepsearch/News/qa_formatted.json --images_dir=data/mm_deepsearch/News/images --local_dir=data/mm_deepsearch/livevqa_news_qw3vl

# Process all splits with custom sizes
python examples/data_preprocess/mm_search/livevqa_news_qw3vl.py --qa_json_path=data/mm_deepsearch/News/qa_formatted.json --images_dir=data/mm_deepsearch/News/images --local_dir=data/mm_deepsearch/livevqa_news_qw3vl --train_size=900 --val_size=20 --test_size=80

# Process only train split
python examples/data_preprocess/mm_search/livevqa_news_qw3vl.py --qa_json_path=data/mm_deepsearch/News/qa_formatted.json --images_dir=data/mm_deepsearch/News/images --split=train --local_dir=data/mm_deepsearch/livevqa_news_qw3vl

# Process only test split
python examples/data_preprocess/mm_search/livevqa_news_qw3vl.py --qa_json_path=data/mm_deepsearch/News/qa_formatted.json --images_dir=data/mm_deepsearch/News/images --split=test --local_dir=data/mm_deepsearch/livevqa_news_qw3vl

# Process with content length filter
python examples/data_preprocess/mm_search/livevqa_news_qw3vl.py --qa_json_path=data/mm_deepsearch/News/qa_formatted.json --images_dir=data/mm_deepsearch/News/images --local_dir=data/mm_deepsearch/livevqa_news_qw3vl --filter_len=8192

# Process only 10 examples (useful for testing)
python examples/data_preprocess/mm_search/livevqa_news_qw3vl.py --qa_json_path=data/mm_deepsearch/News/qa_formatted.json --images_dir=data/mm_deepsearch/News/images --local_dir=data/mm_deepsearch/livevqa_news_qw3vl --max_examples=10

# Use a different Qwen3VL model
python examples/data_preprocess/mm_search/livevqa_news_qw3vl.py --qa_json_path=data/mm_deepsearch/News/qa_formatted.json --images_dir=data/mm_deepsearch/News/images --local_dir=data/mm_deepsearch/livevqa_news_qw3vl --model_name=Qwen/Qwen3-VL-72B-Instruct
"""
