...
# Multimodal Deep Research Training Configuration - 7B Model (TEST VERSION)
# Optimized for Qwen2.5-VL-7B with better capacity for ReAct pattern following
# 7B model should have better instruction following and tool use capabilities
...
dataset_name=mm_deepsearch/livevqa_news  # Using MMSearch-Plus data for multimodal_deepsearch training
train_data=[$(pwd)/data/${dataset_name}/train.parquet]
val_data=[$(pwd)/data/${dataset_name}/val.parquet]  # Using test split as validation data
model_name=Qwen/Qwen2.5-VL-7B-Instruct  # 7B parameter vision-language model for better multimodal capabilities
rl_alg=grpo # gae(ppo) or grpo, if grpo, then better set n>1 otherwise the group norm can not be effective
n_gpus_per_node=8  # Use 8 GPUs for 7B model
n_nodes=1
# DEBUG MODE: Reduced rollout counts and batch sizes for debugging
...
n=2  # Number of rollouts per prompt (reduced for debugging)
batch_size=8  # Number of prompts per batch (will be divided by 8 GPUs = 1 per GPU minimum)
ppo_mini_batch_size=4  # PPO mini-batch size (reduced for debugging, must be >= batch_size/n_gpus_per_node)
# Total rollouts per batch = n * batch_size = 2 * 8 = 16
...
strategy="fsdp2"  # Use fsdp2 for 7B model
...
lr=2e-6  # Learning rate for 7B model
reward_manager=multimodal_deepsearch  # Multimodal deep research reward manager
ppo_micro_batch_size_per_gpu=1  # Must be <= normalized ppo_mini_batch_size (4/8=0.5, but minimum 1)
log_prob_micro_batch_size_per_gpu=1  # Must be <= normalized batch_size per GPU (8/8=1)
tensor_model_parallel_size=2  # Use 2 for better memory management with images
gpu_memory_utilization=0.80  # Reduced for debugging to prevent OOM
...
run_name_postfix="mmresearch-7b-test"
