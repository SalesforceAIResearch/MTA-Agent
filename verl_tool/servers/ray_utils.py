...
class RayToolManager:
    ...
    def _identify_tool_for_action(self, action: str, extra_field: Dict[str, Any]) -> Optional[str]:
        ...
        # Note: <tool_call> wrapper doesn't interfere with regex matching for tool tags like
        # <image_search_text>, <text_search_text>, etc. The regex patterns work regardless.
        # However, tools that specifically look for <tool_call> tags (like ipython_code, mcp_interface)
        # might fail if the tag is incomplete, but that's fine - we'll try other tools.
        
        logger.debug(f"Attempting to identify tool for action. Available tools: {standard_tools}")
        logger.debug(f"Action preview (first 300 chars): {action[:300]}")
        if "<tool_call>" in action:
            logger.debug(f"Action contains <tool_call> tag (complete: {'</tool_call>' in action})")
        
        parse_errors = []
        for tool_type in standard_tools:
            try:
                tool = self.tools[tool_type]
                # For Ray actors, we can't use hasattr reliably, so just try calling parse_action
                try:
                    result = ray.get(tool.parse_action.remote(action), timeout=5.0)
                    _, valid = result
                    if valid:
                        logger.debug(f"Tool {tool_type} successfully parsed action")
                        return tool_type
                    else:
                        logger.debug(f"Tool {tool_type} parse_action returned valid=False")
                except AttributeError:
                    # Tool doesn't have parse_action method
                    logger.debug(f"Tool {tool_type} does not have parse_action method")
                    continue
                except ray.exceptions.GetTimeoutError:
                    parse_errors.append((tool_type, "Timeout after 5.0s"))
                    logger.warning(f"Tool {tool_type} parse_action timed out")
                    continue
            except Exception as e:
                parse_errors.append((tool_type, str(e)))
                logger.debug(f"Tool {tool_type} parse error: {e}")
                continue
        
        # Log summary if no tool was found
        if parse_errors:
            logger.warning(f"Failed to parse action with {len(parse_errors)} tools. Errors: {parse_errors[:3]}")
        # Check if action contains recognizable tool tags
        tool_tags = ["<text_search_text>", "<text_search_image>", "<web_read>", "<python>", "<bash>", "<image_search_text>"]
        has_tool_tag = any(tag in action for tag in tool_tags)
        if not has_tool_tag:
            logger.debug(f"Action does not contain any recognizable tool tags. Action preview: {action[:200]}")
        else:
            # Action has tool tags but no tool matched - this is suspicious
            found_tags = [tag for tag in tool_tags if tag in action]
            logger.warning(f"Action contains tool tags {found_tags} but no tool matched! Action preview: {action[:300]}")
