...
@register_tool
class GoogleSearchTavilyTool(BaseTool):
    """
    Async Google search tool using Tavily API with proper cleanup.
    """
    
    tool_type = "google_search_tavily"
    ...
    def get_usage_inst(self):
        ...
        return "Search the web using Tavily API. Use <search_text>your query</search_text> format."
    ...
    async def _conduct_action_async(self, trajectory_id: str, action: str, extra_field: Dict[str, Any]) -> Tuple[str, bool, bool]:
        ...
        if not is_valid:
            observation = "Invalid search query. Use <search_text>your query</search_text> format."
            done, valid = False, False
        else:
...
if __name__ == "__main__":
    ...
    # Test basic search
    tool = GoogleSearchTavilyTool(api_key=api_key, max_results=2)
    
    # Test query parsing
    query, valid = tool.parse_action("<search_text>Python programming</search_text>")
    print(f"Parsed query: '{query}', Valid: {valid}")
    
    # Test search execution
    observation, done, valid = tool.conduct_action("test", "<search_text>AI news</search_text>", {})
    print(f"Search result - Valid: {valid}, Done: {done}")
