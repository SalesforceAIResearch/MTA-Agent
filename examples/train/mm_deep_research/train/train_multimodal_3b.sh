...
# Multimodal Deep Research Training Configuration - 3B Model
...
n_nodes=1
n=8  # Reduced from 16 for 3B model
batch_size=64  # Reduced from 128 for 3B model
ppo_mini_batch_size=16  # Reduced from 32 for 3B model
...
ppo_micro_batch_size_per_gpu=2  # Increased from 1 for 3B model
log_prob_micro_batch_size_per_gpu=16  # Increased from 8 for 3B model
...
run_name_postfix="multimodal-research-3b"
...
python -m verl_tool.servers.serve --host $host --port $port --tool_type "web_text_to_text_search,web_text_to_img_search,web_url_reader,web_image_to_text,ipython_code,bash_terminal" --workers_per_tool 4 --use_ray True &
server_pid=$!
