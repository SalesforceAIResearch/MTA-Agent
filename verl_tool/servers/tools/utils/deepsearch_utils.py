...
def extract_relevant_info_tavily(search_results):
    """
    Extract relevant information from Tavily search results.

    Args:
        search_results (dict): JSON response from the Tavily API.
    ...
    if 'results' in search_results and search_results['results']:
        for id, result in enumerate(search_results['results']):
            info = {
                ...
                'snippet': result.get('content', '')[:500],  # Use content as snippet, truncated
                'context': ''  # Reserved field to be filled later
            }
            useful_info.append(info)
